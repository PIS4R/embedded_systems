...
int main(void)
{
  ...
  while (1)
  {
	...
	//if(counter == 125){ //HAL_GetTick()%500 == 0 //(counter / (int)(0.5 * 100)) % 2
	...
	//user_JOY_input_handle();

	if(counter == DELAY){
		if(NumberLED > 0){
			HAL_GPIO_WritePin(ports[NumberLED-1], diodes[NumberLED-1], GPIO_PIN_RESET);
		}
		else{
			HAL_GPIO_WritePin(ports[7], diodes[7], GPIO_PIN_RESET);
		}

		HAL_GPIO_WritePin(ports[NumberLED], diodes[NumberLED], GPIO_PIN_SET);
		NumberLED = (NumberLED + 1) % 8;
	}
...
static void MX_GPIO_Init(void)
{
  ...
  GPIO_InitTypeDef GPIO_InitStruct = {0};
    ...
    GPIO_InitStruct.Pin = LED_3;
    GPIO_InitStruct.Mode = GPIO_MODE_OUTPUT_PP;
    HAL_GPIO_Init(GPIOC, &GPIO_InitStruct);

    GPIO_InitStruct.Pin = LED_4;
    GPIO_InitStruct.Mode = GPIO_MODE_OUTPUT_PP;
    HAL_GPIO_Init(GPIOC, &GPIO_InitStruct);

    GPIO_InitStruct.Pin = LED_5;
    GPIO_InitStruct.Mode = GPIO_MODE_OUTPUT_PP;
    HAL_GPIO_Init(GPIOE, &GPIO_InitStruct);

    GPIO_InitStruct.Pin = LED_6;
    GPIO_InitStruct.Mode = GPIO_MODE_OUTPUT_PP;
    HAL_GPIO_Init(GPIOD, &GPIO_InitStruct);

    GPIO_InitStruct.Pin = LED_7;
    GPIO_InitStruct.Mode = GPIO_MODE_OUTPUT_PP;
    HAL_GPIO_Init(GPIOE, &GPIO_InitStruct);

    GPIO_InitStruct.Pin = LED_8;
    HAL_GPIO_Init(GPIOE, &GPIO_InitStruct);
    GPIO_InitStruct.Mode = GPIO_MODE_OUTPUT_PP;
    HAL_GPIO_Init(GPIOC, &GPIO_InitStruct);
